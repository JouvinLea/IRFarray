...
if __name__ == '__main__':
    ...
    print "Store the IRF in a 4D table from the MC simulations in ",  os.path.expandvars('$HESSCONFIG') , " and for the config ", results.config
    ...
    filehistoarea = os.path.expandvars('$HESSCONFIG') + "/" + results.config + "/gFixedEnergy_paris_0-8-8-8_CamOptimal_hwtrig_eff100/CollectionArea.root"
    filehistoresol = os.path.expandvars('$HESSCONFIG') + "/" + results.config + "/gFixedEnergy_paris_0-8-8-8_CamOptimal_hwtrig_eff100/EnergyResolution.root"
    ...
    for (ieff, eff) in enumerate(effMC):
        ...
        for (ioff, off) in enumerate(offMC):
            ...
            for (izen, zen) in enumerate(zenMC):
                print zen
                filehistoarea =  os.path.expandvars('$HESSCONFIG') + "/" + results.config + "/gFixedEnergy_paris_0-8-8-8_CamOptimal_hwtrig_eff" + str(
                    eff) + "/CollectionArea.root"
                filehistoresol =  os.path.expandvars('$HESSCONFIG') + "/" + results.config + "/gFixedEnergy_paris_0-8-8-8_CamOptimal_hwtrig_eff" + str(
    ...
    outdir=os.path.expandvars('$HESSCONFIG')+"/"+results.config
